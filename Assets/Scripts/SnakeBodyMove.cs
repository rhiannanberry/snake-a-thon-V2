...
public class SnakeBodyMove : MonoBehaviour {
    ...
    void Start () {
        ...
        List<GameObject> parts = head.GetComponent<Snake>().bodyParts;
        if (parts.Count == 0)
        {
            parent = head;
        } else
        ...
        head.GetComponent<Snake>().bodyParts.Add(gameObject);
    }
    ...
    void Update () {
        ...
        float speed = head.GetComponent<Snake>().currentSpeed;
